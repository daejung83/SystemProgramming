...
/**
Returns an array of timestamp_entry's between the given timestamp range [start,
end) (in order by timestamp).
Upon completion the value of return_size is set to the size of the returned
array.
The selector is a function pointer to a function that takes in a void pointer,
and returns
a int, (0 or 1). This function is passed the data passed in, and is returns 1 if
this
value should be included in the return value. This essientially lets you choose
the values
from the queue you would like to gather within the range.
*/
timestamp_entry *queue_gather(queue_t *queue, unsigned long start,
                              unsigned long end, int (*selector)(void *),
                              int *return_size);
