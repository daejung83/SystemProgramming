...
int compare_timestamp(const void *v1, const void *v2) {
  return ((timestamp_entry *)v1)->timestamp_ <
         ((timestamp_entry *)v2)->timestamp_;
}

timestamp_entry *queue_gather(queue_t *queue, unsigned long start,
                              unsigned long end, int (*selector)(void *),
                              int *return_size) {
  if (return_size == NULL || selector == NULL)
    return NULL;
  ...
  queue_entry_t *entry = queue->head_;
  while (entry != NULL) {
    if (entry->stamped_entry_.timestamp_ >= start &&
        entry->stamped_entry_.timestamp_ < end &&
...
int queue_at(queue_t *queue, int entry_index, timestamp_entry *result) {
  if (entry_index >= queue->size_)
    return 0;

  size_t i;
  queue_entry_t *entry = queue->head_;
  for (i = 0; i < (size_t)entry_index; i++)
    entry = entry->next_;
...
int queue_size(queue_t *queue) { return queue->size_; }
